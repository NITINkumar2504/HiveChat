...
export function getInitials(name) {
  return name
    .split(" ")
    .filter(Boolean)   // filter(Boolean) is a handy shorthand trick used to remove all falsy values from an array, leaving only truthy ones.
                       // filter(Boolean) does the exact same thing as writing .filter(item => Boolean(item)) or .filter(item => item)
